...
GinsengAI.prototype.scoreIsGood = function(scores, goodScores, depth) {
	if (depth <= 0) {
		debug("SCORE IS GOOD")
		return true;
	}
...
GinsengAI.prototype.getMoveScore = function(origGame, move, depth) {
	debug("Depth: " + depth);
...
GinsengAI.prototype.getHighestScore = function(origGame, newGame, highScore, depth) {
	// magic
...
GinsengAI.prototype.calculateScore = function(origGame, copyGame) {
	// Simple move scoring...
	var score = 0;
...
GinsengAI.prototype.getPossibleMoves = function(thisGame, player) {
	var moves = [];
...
GinsengAI.prototype.addAccentSelectionMoves = function(moves, game) {
...
GinsengAI.prototype.addPlantMoves = function(moves, game, player) {
...
GinsengAI.prototype.addArrangeMoves = function(moves, game, player) {
...
GinsengAI.prototype.getCurrentPlayerForGame = function(game, notation) {
...
GinsengAI.prototype.getNotation = function(boardPoint) {
	return new RowAndColumn(boardPoint.row, boardPoint.col).notationPointString;
};

GinsengAI.prototype.getStartPoints = function(game, player) {
	var points = [];
...
GinsengAI.prototype.getPossibleMovePoints = function(game) {
	var points = [];
	for (var row = 0; row < game.board.cells.length; row++) {
		for (var col = 0; col < game.board.cells[row].length; col++) {
...
GinsengAI.prototype.getBasicFlowerTileCode = function(game) {
...
GinsengAI.prototype.tileContainsBasicFlower = function(tilePile) {
	for (var i = 0; i < tilePile.length; i++) {
		if (tilePile[i].type === BASIC_FLOWER) {
			return true;
		}
...
GinsengAI.prototype.getTilePile = function(game, player) {
...
GinsengAI.prototype.isOpenGate = function(game) {
...
GinsengAI.prototype.getOpponent = function() {
...
GinsengAI.prototype.getNumHarmoniesForPlayer = function(game, player) {
	return game.board.harmonyManager.numHarmoniesForPlayer(player);
};

GinsengAI.prototype.getNumTilesInGardensForPlayer = function(game, player) {
	return game.board.numTilesInGardensForPlayer(player);
};

GinsengAI.prototype.getNumTilesOnBoardForPlayer = function(game, player) {
	return game.board.numTilesOnBoardForPlayer(player);
};

GinsengAI.prototype.getSurroundness = function(game, player) {
	return game.board.getSurroundness(player);
};

GinsengAI.prototype.getLongestHarmonyRingLength = function(game, player) {
	return game.board.harmonyManager.ringLengthForPlayer(player);
};

GinsengAI.prototype.getNumHamoniesCrossingCenter = function(game, player) {
	return game.board.harmonyManager.getNumCrossingCenterForPlayer(player);
};

GinsengAI.prototype.ensurePlant = function(move, game, player) {
